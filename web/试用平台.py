...
def analyze_data(prediction_type):
    ...
    if "uploaded_data" in st.session_state and st.session_state['uploaded_data'] is not None:
        ...
        if prediction_type == "单轴抗压强度":
            scaler_x = load_scaler('model/scaler_x.pkl1')
            scaler_y = load_scaler('model/scaler_y.pkl1')
            model_filename = 'model/rock_ucs_model.pkl1'
            prediction_column = 'UCS_预测值（MPa）'
        elif prediction_type == "粘聚力":
            scaler_x = load_scaler('model/scaler_x.pkl3')
            scaler_y = load_scaler('model/scaler_y.pkl3')
            model_filename = 'model/rock_c_model.pkl1'
            prediction_column = '粘聚力_预测值（MPa）'
        elif prediction_type == "内摩擦角":
            scaler_x = load_scaler('model/scaler_x.pkl2')
            scaler_y = load_scaler('model/scaler_y.pkl2')
            model_filename = 'model/rock_f_model.pkl1'
